...
def main_page():
    ...
    if title: #remove news widgets
        remove_news_article(title)
        return redirect("/index")

    #reading data from files t o send to webpage
    ...
    news_data=[]
    #inserting links to article source sites
    for item in news:
        ...
        if len(news_data)>=8: # limits article widgets
            break
    #reading the covid data  to send to website 
    #data is read from a file to avoid polling the API
    ...
    for item in queue_covid:
        ...
        if item[2] == schedule_covid_updates:
            #if its a repeated covid update
            try:
                chronology=names[0]
    ...
    if remove_event:
        for entry in update_data:
            if entry["title"]==remove_event:
                #cancelling event from schedule
                try:
        ...
        for entry in list_of_repeated_updates: #REMOVES repeated updates from both queues
            if entry["title"]==remove_event:
                ...
                try:
                    ...
                    #events field has 2 events- this cancels both
                    #one event is to schedule an update
                    #the other is to schedule the reschedule function
    ...
    if len(update_data)==0 and len(list_of_repeated_updates)!=0:
        #in case there schould be no updates, but repeated
        #updates haven't been cancelled yet
        logging.info("wiped list_of_repeated_updates and cancelled all events")
        list_of_repeated_updates.clear()
        #cancels all items in the schedules
        ...
        if valid_name:
            ...
            if time_input:
                ...
                if update_covid_data:
                    #adding to the list of update events
    ...
    #writing updates to file so they can be used if the code 
    #stops running
